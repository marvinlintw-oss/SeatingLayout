...
export const ManualAssign: React.FC = () => {
  const { personnel, activeSessionId, sessions } = useProjectStore();
  ...
  const absentPersonnel = personnel.filter(p => !isAttending(p));

  const seated = attendingPersonnel.filter(p => seatedIdsInCurrentSession.has(p.id));
  const unseated = attendingPersonnel.filter(p => !seatedIdsInCurrentSession.has(p.id));
    ...
    <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 custom-scrollbar pb-12">
      ...
      <div>
        ...
        <div className="space-y-2">
            {unseated.map(person => (
                <div 
                    key={person.id} draggable onDragStart={(e) => handleDragStart(e, person.id)} 
